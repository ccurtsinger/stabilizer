...
private:
	...
	Value* getLocalSlot(size_t index, Instruction *insertion_point, Twine name) {
		...
		GetElementPtrInst *slot = GetElementPtrInst::Create(table, indices.begin(), indices.end(), name+".ptr", insertion_point);
	...
	GlobalVariable* getNameString() {
		if(name_str == NULL) {
			Constant *str_init = ConstantArray::get(m->getContext(), base->getName());
	...
	GlobalVariable* getReferencesTable() {
		if(references_table == NULL) {
			references_table = MakeTable(*m, ("stabilizer."+base->getName()+"_references").getSingleStringRef(), references);
		}
...
public:
	...
	void makeUseLocal(Use &use) {
		...
		if(gv != NULL && /*gv != base &&*/ insertion_point != NULL) {
			if(PHINode *phi = dyn_cast<PHINode>(insertion_point)) {
	...
	GlobalVariable* getInfo() {
		if(info == NULL) {
			...
			vals.push_back(ConstantExpr::getBitCast(getNameString(), Type::getInt8PtrTy(m->getContext())));
			vals.push_back(ConstantExpr::getBitCast(base, Type::getInt8PtrTy(m->getContext())));
			vals.push_back(ConstantExpr::getBitCast(next, Type::getInt8PtrTy(m->getContext())));
			vals.push_back(ConstantExpr::getBitCast(getReferencesTable(), Type::getInt8PtrTy(m->getContext())));
			
			Constant *info_init = ConstantStruct::get(m->getContext(), vals, false);
	...
	Type* getRandomAllocaStructType() {
		...
		return StructType::get(m->getContext(), elements, false);
	}
};
...
struct StabilizerPass : public ModulePass {
	...
	virtual bool runOnModule(Module &m) {
		...
		if(stabilize_stack) {
			...
			Function::Create(
				FunctionType::get(Type::getInt8PtrTy(m.getContext(), 0), params, false),
				Function::ExternalLinkage,
		...
		if(stabilize_code) {
			...
			vector<const Type*> register_global_params;
			...
			Function *register_g = Function::Create(
				 FunctionType::get(Type::getVoidTy(m.getContext()), register_global_params, false),
				 Function::ExternalLinkage,
			...
			if(stabilize_globals) {
				for(set<GlobalVariable*>::iterator g_iter = randomized_globals.begin(); g_iter != randomized_globals.end(); g_iter++) {
					...
					if(!g->getName().equals("llvm.global_ctors") && !g->getName().equals("llvm.used") && !g->isConstant()) {
						...
						CallInst::Create(register_g, args.begin(), args.end(), "", ctor_bb);
					} else {
			...
			for(set<Function*>::iterator f_iter = stabilized_functions.begin(); f_iter != stabilized_functions.end(); f_iter++) {
				...
				if(!f->isDeclaration()) {
					...
					CallInst::Create(register_fn, args.begin(), args.end(), "", ctor_bb);
				}
			}
		}
	...
	GlobalVariable* makeFrameTable(Module &m, Function *f, size_t num_frames = 256) {
		...
		StructType *entry_type = StructType::get(m.getContext(), locals_type, locals_type, NULL);
		...
		for(size_t i=0; i<num_frames; i++) {
			...
			vals.push_back(ConstantStruct::get(m.getContext(), entry_vals, false));
		}
	...
	void randomizeStack(Module &m, Function *f, GlobalVariable *depth_global, GlobalVariable *frame_table_global) {
		...
		CallInst *new_frame_i8p = CallInst::Create(
			...
			relocate_frame_args.begin(),
			relocate_frame_args.end(),
			"",
			get_frame
		);
		...
		PHINode *frame_struct = PHINode::Create(PointerType::get(s->getRandomAllocaStructType(), 0), "", set_stack);
